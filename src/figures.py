...
yolo_coco_mood = pd.read_csv(r'tables/yolo_coco_mood.csv')
...
ax[0][3].bar(x_axis, grounding_sam_with_prompting.iloc[9].to_list()[1:])
ax[0][3].set_title('grounding sam with prompting')
ax[1][0].bar(x_axis, grounding_dino.iloc[9].to_list()[1:])
ax[1][0].set_title('grounding dino')
ax[1][1].bar(x_axis, grounding_dino_with_prompting.iloc[9].to_list()[1:])
ax[1][1].set_title('grounding dino with prompting')
ax[1][2].bar(x_axis, yolo_coco_mood.iloc[9].to_list()[1:])
ax[1][2].set_title('yolo coco mood')
plt.show()
plt.close()
...
ax[2].bar(x_axis, yolo_coco_mood.iloc[9].to_list()[1:])
plt.show()
...
print(yolo_coco_mood.to_html(index = False))
